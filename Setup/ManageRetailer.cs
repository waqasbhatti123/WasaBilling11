...
namespace FOS.Setup
{
    public class ManageRetailer
    {
        ...
        public static List<RetailerData> GetRetailerLocations(int RegionalHeadID, int DealerID, int SaleOfficerID, int RegionID, int CityID, string ZoneID,string untaggedOnly = null)
        {
            List<RetailerData> retailerData = new List<RetailerData>();
            ...
            //List<RetailerData> retailerData = new List<RetailerData>();
            //var today = DateTime.UtcNow.AddHours(5);
            //var month = new DateTime(2022,06,01);
            //var first = month.AddMonths(1);
            ...
            try
            {
                using (FOSDataModel dbContext = new FOSDataModel())
                {
                    ...
                    retailerData = dbContext.sp_GetPins(CityID, ZoneID)
                             .Select(
                                 u => new RetailerData
                                 {
                                     ID = Convert.ToInt32(u.CID),
                                     Name = u.ConsumerName,
                                     ShopName = u.F_Name,
                                     ...
                                     AreaName = u.WARD__NO_,
                                     //TotalConsumers = dbContext.TBL_Consumers.Where(x => x.DDRID == CityID && x.WardID == ZoneID).Count(),

                                     //                 //retailerData = dbContext.JobsDetails.Where(u => u.Job.CityID == CityID && u.Job.ZoneID == ZoneID
                                     //                 //         && u.JobDate >= StartDate && u.JobDate <= EndDate
