        ...
        public IHttpActionResult Get(int SOID)
        {
            ...
            try
            {
                ...
                if (SOID > 0)
                {
                    ...
                    var result = dbContext.TBL_RouteSelection.Where(r => r.SOID == SOID && r.Createddate >= dtFromToday && r.Createddate <= dtToToday && r.IsActive == true).Select(x => new
                    {
                        ID=x.ID,
                        CityID= x.DDRID,
                        AreaID = x.WardID,
                        SubDevisionID = x.AreaID,
