    ...
    public class RouteSelectionController : ApiController
    {
        ...
        { // This controller is for retailers orders.
            ...
            try
            {
                ...
                var data = db.TBL_RouteSelection.Where(r => r.SOID == rm.SOID && r.Createddate>= dtFromToday && r.Createddate<= dtToToday && r.IsActive==true).FirstOrDefault();
                if (data == null)
                {
                    TBL_RouteSelection jobDet = new TBL_RouteSelection();

                    jobDet.SOID = rm.SOID;
                    jobDet.DDRID = rm.CityID;
                    jobDet.WardID = rm.AreaID;
                    jobDet.AreaID = rm.SubDivisionID;
                    jobDet.IsActive = true;
                    jobDet.Createddate = DateTime.UtcNow.AddHours(5);
